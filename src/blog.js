...
export default class blog extends React.Component {
  constructor(props) {
    ...
    console.log("constructor", this.state.title);
  }
  ...
  componentWillMount() {
    ...
    const itemId = navigation.getParam("docid");
    console.log("params from the blogadd screen component will Mount", itemId);
    this.setState({ title: itemId });
    firebase
      .firestore()
      .collection("blogs")
      .doc(itemId)
      ...
      .then(doc => {
        if (doc.exists) {
          ...
          var con = blogdata.content;
          this.setState({ content: con });

          console.log("school data " + JSON.stringify(blogdata));
          // this.setState({schoolname:schoolname});
          // console.log("school ..............name"+schoolname)
          // setstate();
          //  navigate('notifyrange');
          // this.props.navigation.navigate('notifyrange');
          // nextscreen();
        } else {
          Alert.alert("School ", "School id Does not exist");
          console.log("Documents does not exist");
        }
      });
  }
  ...
  addcomment=()=>{
    ...
    .set({ comments: this.state.comments},{merge:true}).then((docref)=>{
  ...
  render() {
    const { navigation } = this.props;
    const itemId = navigation.getParam("docid");
    console.log("params from the blogadd screen", itemId);
                ...
                <CardItem>
                  ...
                  <Right>
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        this.props.navigation.navigate("blogadd", {
                          docid: this.state.title
                        })
                      }
                    >
                      <Text>Edit</Text>
          ...
          <Container>
            <Content>
              <Card>
                <CardItem>
                  <Body>
                    <Text style={{fontSize: 19}}>{this.state.content}</Text>
                  </Body>
                </CardItem>
              </Card>
            </Content>
